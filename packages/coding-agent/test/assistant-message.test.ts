...
describe("AssistantMessageComponent", () => {
	...
	test("uses configured output padding for user messages", () => {
		...
		// UserMessageComponent.render wraps the content in a box border, so the
		// body lines start with "│ " (border + interior space). outputPad adds
		// an additional leading space inside the border, producing two spaces
		// between "│" and the content when outputPad=1, and one space when 0.
